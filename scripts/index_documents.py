...
def _print_chunk_stats(all_chunks: list[dict]) -> None:
    ...
    print(f"    Tokens por chunk  : min={min_tokens}  média={avg_tokens:.0f}  max={max_tokens}")
...
def main() -> None:
    ...
    for doc in documents:
        ...
        logger.debug(
            "'%s'%s: %d chunks.",
            Path(doc["source"]).name, page_info, len(chunks)
        )

    logger.info("%d chunks criados a partir de %d entrada(s).", len(all_chunks), len(documents))
    ...
    print(f"  Tokens estimados     : {sum(count_tokens(c['text']) for c in all_chunks):,}")
