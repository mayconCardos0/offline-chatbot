...
from rag.chunking import chunk_document, ChunkConfig, count_tokens
...
Marianne é a personificação da República Francesa e um símbolo de liberdade.
    ...
    for i, chunk in enumerate(chunks):
        ...
        if any(title in chunk['text'] for title in ['Primavera dos Povos', 'Três Dias Gloriosos', 'Marianne']):
...
Suas conquistas militares transformaram a Europa no início do século XIX.
    ...
    for i, chunk in enumerate(chunks):
        ...
        required_fields = ['source', 'page', 'section', 'chunk_id', 'chapter', 'topic', 'section_title']
